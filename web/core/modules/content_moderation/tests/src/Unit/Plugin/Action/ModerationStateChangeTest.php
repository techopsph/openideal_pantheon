...
class ModerationStateChangeTest extends UnitTestCase {
  ...
  public function testAccessModerationStateChange(ModerationInformationInterface $moderation_info, $node, $result, AccountInterface $account = NULL) {
    ...
    $plugin = $this->getModerationStateChangeMock($config, $moderation_info, $node);
   ...
   */
  protected function getModerationStateChangeMock($config, $moderation_info, $node) {
    $plugin = $this->getMockBuilder(ModerationStateChange::class)
      ->setConstructorArgs([$config, 'moderation_state_change', ['type' => 'node'], $moderation_info, $this->entityTypeManager, $this->currentUser])
